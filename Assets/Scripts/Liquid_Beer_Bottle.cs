...
[ExecuteInEditMode]
public class Liquid_Beer_Bottle : MonoBehaviour
{
    ...
    float MapAngleToValue()
    {
        ...
        if (isInCollider) // Scale for other Fluid to fill up
        {
            ...
            float lastScale = Fluid.GetComponent<Liquid_Beer_Bottle>().CurrentFillAmount;
            float scale = 0.66f - (currentValue - MinFillAmount) * 5f;
            float highestScale = Mathf.Min(scale, lastScale);
            Fluid.GetComponent<Liquid_Beer_Bottle>().CurrentFillAmount = highestScale;
        }
